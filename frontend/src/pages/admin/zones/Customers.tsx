...
export default function Customers() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      setEditing(null);
      api.get<Customer[]>('/admin/customers').then(setItems).catch(console.error);
    } catch (err: any) {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await api.del(`/admin/customers/${id}`);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl text-navy">Clientes</h1>
          <p className="text-sm text-ink-soft">Gestión de cartera de clientes</p>
        </div>
        <Button onClick={() => setEditing({ ...empty } as any)}>+ Nuevo cliente</Button>
      </div>
      <div className="card">
        {loading ? (
          <p className="text-ink-soft py-6 text-center">Cargando...</p>
        ) : items.length === 0 ? (
          <p className="text-ink-soft py-6 text-center">No hay clientes registrados</p>
        ) : (
      ...
      <Modal open={!!editing} title={editing?.id ? 'Editar cliente' : 'Nuevo cliente'} onClose={() => setEditing(null)}>
        {editing && (
