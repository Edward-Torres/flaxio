...
import type { Return, Product, Supplier } from '../../../lib/types';
...
export default function Returns() {
  ...
  const [editing, setEditing] = useState<Partial<Return> | null>(null);
  ...
  const empty = {
    ...
    items: [] as { productId: string; quantity: number; listPrice: number }[],
  };

  useEffect(() => {
    Promise.all([
      api.get<Return[]>('/admin/returns'),
      api.get<Product[]>('/admin/products'),
      api.get<Supplier[]>('/admin/suppliers'),
    ])
      .then(([returns, prods, sups]) => {
        setItems(returns);
        setProducts(prods);
        setSuppliers(sups);
      })
  ...
  function addItem() {
    ...
    setEditing({
      ...editing,
      items: [...(editing.items || []), { productId: '', quantity: 1, listPrice: 0 }],
    } as any);
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing.id) {
        await api.put(`/admin/returns/${editing.id}`, payload);
      } else {
        await api.post('/admin/returns', payload);
      }
      setEditing(null);
      api.get<Return[]>('/admin/returns').then(setItems).catch(console.error);
    } catch (err: any) {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await api.del(`/admin/returns/${id}`);
  ...
  const total = editing?.items?.reduce((sum: number, item: any) => sum + item.listPrice * item.quantity, 0) || 0;

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl text-navy">Devoluciones</h1>
          <p className="text-sm text-ink-soft">Registro de devoluciones a proveedores</p>
        </div>
        <Button onClick={() => setEditing({ ...empty } as any)}>+ Nueva devolución</Button>
      </div>
      <div className="card">
        {loading ? (
          <p className="text-ink-soft py-6 text-center">Cargando...</p>
        ) : items.length === 0 ? (
          <p className="text-ink-soft py-6 text-center">No hay devoluciones registradas</p>
        ) : (
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.id} className="border-b border-line/50">
                  <td className="p-3">{new Date(r.date).toLocaleDateString('es-AR')}</td>
                  <td className="p-3">{r.supplier}</td>
                  <td className="p-3 text-right">${r.total.toFixed(2)}</td>
                  <td className="p-3">
                    <span className={'badge ' + (r.status === 'PENDING' ? 'yellow' : 'green')}>{r.status}</span>
                  </td>
                  <td className="p-3 text-right">
                    <div className="flex gap-2 justify-end">
                      <Button variant="ghost" onClick={() => setEditing(r as any)} className="!px-2 !py-1">Editar</Button>
                      <Button variant="ghost" onClick={() => handleDelete(r.id)} className="!px-2 !py-1 text-danger">Eliminar</Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal open={!!editing} title={editing?.id ? 'Editar devolución' : 'Nueva devolución'} onClose={() => setEditing(null)} wide>
        {editing && (
                    ...
                    <div key={idx} className="grid grid-cols-12 gap-2 items-end">
                      ...
                      <div className="col-span-3">
                        <Input
                          ...
                          onChange={(e) => updateItem(idx, 'listPrice', parseFloat(e.target.value) || 0)}
                          required
                        />
                      </div>
                      <div className="col-span-2">
